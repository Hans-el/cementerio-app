...
export class TramitesComponent implements OnInit {
  ...
  toggleDetalle(solicitud: Solicitud): void {
    ...
    if (!this.documentosCache[id]) {
      ...
      this.tramiteService.getDocumentos(id).subscribe({
        next: (docs) => {
          this.documentosCache[id] = docs;
          this.cargandoDocumentos[id] = false;
        },
        error: () => {
          this.documentosCache[id] = [];
          this.cargandoDocumentos[id] = false;
        },
      });
    }
  ...
  iconoPorTipo(nombre: string): string {
    ...
    if (n.includes('inhumación') || n.includes('inhumacion'))
      return 'bi-file-earmark-arrow-up';
    if (n.includes('exhumación') || n.includes('exhumacion'))
      return 'bi-file-earmark-arrow-down';
    if (n.includes('construcción') || n.includes('construccion'))
