...
export class SolicitudTramiteComponent implements OnInit, PuedeSalir {
  ...
  cargarTipo(id_tipo: number): void {
    ...
    this.tramiteService.getTipos().subscribe({
      next: (tipos) => {
        this.tipoTramite =
          tipos.find((t) => t.id_tipo_tramite === id_tipo) ?? null;
        if (!this.tipoTramite) {
          this.router.navigate(['/tramites']);
          return;
        }
      ...
      this.tramiteService
        ...
        .subscribe({
          ...
          error: (err) => {
            Swal.fire(
              'Error',
              err.error?.mensaje || 'No se pudo reemplazar.',
              'error',
            );
            input.value = '';
          },
  ...
  get obligatoriosSubidos(): boolean {
    return this.documentosTipo.every(
      (doc, i) => !doc.obligatorio || this.archivos[i] !== null,
    );
  }
  ...
  enviarSolicitud(): void {
    if (!this.obligatoriosSubidos) {
      Swal.fire(
        ...
        'Debes subir todos los documentos obligatorios.',
        'warning',
      );
      return;
    }
    ...
    Swal.fire({
      ...
      this.tramiteService
        .crearSolicitud(this.tipoTramite!.id_tipo_tramite, archivosAEnviar)
        .subscribe({
          next: () => {
            this.enviando = false;
            ...
            Swal.fire({
              ...
              this.verificarSolicitudActiva(this.tipoTramite!.id_tipo_tramite),
            );
          },
          error: (err) => {
            this.enviando = false;
            Swal.fire(
              'Error',
              err.error?.mensaje || 'No se pudo enviar la solicitud.',
              'error',
            );
          },
        });
