...
export class SolicitudInhumacionComponent {
  ...
  enviarSolicitud(): void {
    if (!this.todosSubidos) {
      Swal.fire('Documentos incompletos', 'Debes subir todos los documentos requeridos.', 'warning');
      return;
    }
    Swal.fire({
  ...
  onReemplazarDocumento(event: Event, doc: DocumentoInhumacion): void {
    ...
    Swal.fire({
      ...
      this.inhumacionService.reemplazarDocumento(doc.id_documento, archivo).subscribe({
        next: () => {
        ...
        error: (err) => {
          Swal.fire('Error', err.error?.message || 'No se pudo reemplazar el documento.', 'error');
          input.value = '';
        },
      });
    });
