...
export class SuperadminComponent implements OnInit {
  ...
  cargarCementerios(): void {
    ...
    this.superadminService.getCementerios().subscribe({
      next: (data) => { this.cementerios = data; this.cargando = false; },
  ...
  guardarCementerio(): void {
    if (this.cementerioForm.invalid) {
      this.cementerioForm.markAllAsTouched(); return;
    }

    const datos = this.cementerioForm.getRawValue();
    const obs$ = this.editandoCementerio
      ? this.superadminService.editarCementerio(this.editandoCementerio.id_cementerio, datos)
    ...
    obs$.subscribe({
      next: () => {
        ...
        Swal.fire({
          ...
          title: this.editandoCementerio ? 'Cementerio actualizado.' : 'Cementerio creado.',
  ...
  abrirModalAdmin(admin?: any): void {
    ...
    if (admin) {
      ...
      this.adminForm.get('contrasena')?.setValidators([Validators.required, Validators.minLength(6)]);
    }
  ...
  guardarAdmin(): void {
    if (this.adminForm.invalid) {
      this.adminForm.markAllAsTouched(); return;
    }
  ...
  eliminarAdmin(admin: any): void {
    Swal.fire({
      ...
      this.superadminService.eliminarAdmin(admin.id).subscribe({
        next: () => {
          this.cargarAdmins();
          Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Admin eliminado.', timer: 2000, showConfirmButton: false });
        },
  ...
  resetContrasena(admin: any): void {
    Swal.fire({
      ...
      this.superadminService.resetContrasena(admin.id, result.value).subscribe({
        next: () => Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Contraseña restablecida.', timer: 2000, showConfirmButton: false }),
        error: () => Swal.fire('Error', 'No se pudo restablecer la contraseña.', 'error'),
      });
    });
  }

  // Helpers
  campo(f: string) { return this.cementerioForm.get(f); }
  campoAdmin(f: string) { return this.adminForm.get(f); }
  cerrarModales(): void { this.modalCementerio = false; this.modalAdmin = false; }
}