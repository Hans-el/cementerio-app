...
export class DashboardComponent implements OnInit {
  ...
  private calcularKPIs(): void {
    ...
    this.totalOcupados = this.resumen.bovedas_ocup + this.resumen.nichos_ocup + this.resumen.cruces_ocup;
  ...
  private renderizarGraficos(): void {
    ...
    const ctxTipos = document.getElementById('chart-tipos') as HTMLCanvasElement;
    if (ctxTipos) {
      this.charts.push(new Chart(ctxTipos, {
        type: 'bar',
        data: {
          ...
          datasets: [
            ...
            {
              label: 'Disponibles',
              data: [r.bovedas - r.bovedas_ocup, r.nichos - r.nichos_ocup, r.cruces - r.cruces_ocup],
    ...
    const ctxDonut = document.getElementById('chart-donut') as HTMLCanvasElement;
    if (ctxDonut) {
      this.charts.push(new Chart(ctxDonut, {
        type: 'doughnut',
        data: {
    ...
    const ctxSectores = document.getElementById('chart-sectores') as HTMLCanvasElement;
    if (ctxSectores) {
      this.charts.push(new Chart(ctxSectores, {
        type: 'bar',
        data: {
          labels: r.sectores.map(s => s.nombre.replace('Cementerio ', '')),
          datasets: [
            { label: 'Ocupados', data: r.sectores.map(s => s.ocupados), backgroundColor: ['#185FA5', '#3B6D11'], borderRadius: 4 },
            { label: 'Disponibles', data: r.sectores.map(s => s.total - s.ocupados), backgroundColor: '#d3d1c7', borderRadius: 4 },
          ],
        },
        options: {
    ...
    const ctxAnios = document.getElementById('chart-anios') as HTMLCanvasElement;
    if (ctxAnios) {
      this.charts.push(new Chart(ctxAnios, {
        type: 'line',
        data: {
